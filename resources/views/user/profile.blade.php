<x-base-page>
    <div class="w-3/5 flex flex-col items-center">        
        <form action="{{ route('user.profile.update') }}" method="POST" class="w-3/5">
            ...
            <h1 class="text-5xl font-bold text-center text-[#666562] mb-6">Profiel</h1>
            ...
            @if(session('success'))
            <p class="text-green-600 my-2 w-full text-center">{{ session('success') }}</p>
            @endif
            
            @if ($errors->any())
                @foreach ($errors->all() as $error)
                    <p class="text-red-600 my-2 w-full text-center">{{ $error }}</p>
                @endforeach
            @endif
            ...
            <div class="flex gap-4 mt-6">
                <!-- Opslaan knop -->
                <button type="submit" class="px-6 py-2 bg-[#519F66] text-white rounded hover:bg-[#3f7c4f]">
                    Opslaan
                </button>
